...
async function generatePlaylistPrompt(model: any, song1: string, song2: string): Promise<Song[]> {
  ...
  return playlist.filter((song): song is Song => song && typeof song === "object")
    .map((song) => ({
...
async function validateSpotifyToken(accessToken: string): Promise<boolean> {
  try {
...
export async function POST(request: Request) {
  ...
  const isValidToken = await validateSpotifyToken(accessToken);
  if (!isValidToken) {
    return Response.json(
  ...
  while (retries <= MAX_RETRIES) {
    try {
      ...
      const playlist = await generatePlaylistPrompt(model, song1, song2);
      ...
      // Filter out songs that weren't found on Spotify (optional)
      const validPlaylist = playlistWithSpotifyIds.filter(song => song.spotifyId);

      return Response.json({
        playlist: validPlaylist,
        originalLength: playlist.length,
        foundOnSpotify: validPlaylist.length
      });
