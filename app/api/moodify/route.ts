import {
  ...
  export async function POST(request: Request) {
    ...
    try {
      // Get access token from cookies instead of request body
      ...
      while (retries <= MAX_RETRIES) {
        try {
          // Get user's recent tracks using the cookie token
          ...
          try {
            ...
            playlist = lines.flatMap((line) => {
              try {
                ...
                return typeof parsedLine === "object" && parsedLine !== null ? [parsedLine] : [];
              } catch (lineError) {
                return [];
              }
            });
